...
### $# meaning the number of parameters passed to the script. So if you pass an argument you can choose between following cases:
while [ "$#" -gt 0 ]; do
  case "$1" in
    -h) HELP=1; break;;
    -i) INPUT="$2"; shift 2;;
...
if [ $SCALE -gt 0 ]; then
    ...
    # check that it's greater than 0.01 i
...
echo "&model
    model_type = 'EVOL'
    file = '/usr/users/jhm1496/stars/44_tau/44_tau/LOGS/$bname'
    file_format = $FORMAT
/
&constants
